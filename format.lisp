...
;; 100個の数字を1行10個ずつに分解している
;; 字寄せシーケンス~<~>の中で~:;制御シーケンスを使い、テキストが等しい長さに分割されるようにしている。
;; 字寄せシーケンスの中では~:;に先立つ文字列(この例では|~%|)は現在のカーソル位置が~:;の2番めのパラメータで指定されるカラム位置を超えていた場合に限り出力される。
;; つまりこの制御シーケンスは33文字分出力したら改行してくれ、と伝えている。
;; 制御文字列|~%|は縦棒、改行、縦棒を出力する。
;; 次の~2dが数字を右寄せで2文字幅で出力する。
